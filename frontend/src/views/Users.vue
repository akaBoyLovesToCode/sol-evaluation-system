  ...
  <div class="users-page">
    ...
    <el-card class="table-card" v-loading="loading">
      <el-table :data="users" style="width: 100%" border>
        ...
        <el-table-column prop="fullName" :label="$t('users.fullName')" min-width="150" />
...
const userFormRules = {
  username: [
    { required: true, message: t('validation.required', { field: t('users.username') }), trigger: 'blur' },
    { min: 3, max: 20, message: t('validation.length', { min: 3, max: 20 }), trigger: 'blur' }
  ],
  fullName: [
    { required: true, message: t('validation.required', { field: t('users.fullName') }), trigger: 'blur' },
    { min: 2, max: 50, message: t('validation.length', { min: 2, max: 50 }), trigger: 'blur' }
  ],
  email: [
    { required: true, message: t('validation.required', { field: t('users.email') }), trigger: 'blur' },
    { type: 'email', message: t('users.invalidEmail'), trigger: 'blur' }
  ],
  role: [
    { required: true, message: t('validation.required', { field: t('users.role') }), trigger: 'change' }
  ],
  password: [
    { required: true, message: t('validation.required', { field: t('users.password') }), trigger: 'blur', when: () => !isEditing.value },
    { min: 8, message: t('validation.minLength', { min: 8 }), trigger: 'blur', when: () => !isEditing.value },
    { 
      pattern: /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d).+$/, 
  ...
  confirmPassword: [
    { required: true, message: t('validation.required', { field: t('users.confirmPassword') }), trigger: 'blur', when: () => !isEditing.value },
    { 
...
const fetchUsers = async () => {
  try {
    ...
    const params = {
      ...
      is_active: searchForm.status === 'active' ? true : 
                searchForm.status === 'inactive' ? false : undefined
    }
