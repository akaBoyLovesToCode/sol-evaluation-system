...
def create_user():
    ...
    try:
        ...
        role = click.prompt('Role', type=click.Choice(['admin', 'group_leader', 'part_leader', 'user']), default='user')
...
def list_users():
    ...
    try:
        ...
        print(f"{'ID':<4} {'Username':<15} {'Email':<25} {'Full Name':<20} {'Role':<15} {'Active':<8}")
        print("-" * 90)
        
        for user in users:
            print(f"{user.id:<4} {user.username:<15} {user.email:<25} {user.full_name:<20} {user.role:<15} {'Yes' if user.is_active else 'No':<8}")
