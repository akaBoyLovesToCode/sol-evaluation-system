...
def test_user_profile_flow(client, session, regular_user, user_headers):
    ...
    response = client.put('/api/users/password', headers=user_headers, json=password_data)
    assert response.status_code == 200
    data = json_response(response)
    assert data['success'] is True
...
def test_evaluation_lifecycle(client, session, regular_user, user_headers, admin_user, admin_headers):
    ...
    response = client.put(f'/api/evaluations/{evaluation_id}', headers=user_headers, json=update_data)
    assert response.status_code == 200
    data = json_response(response)
    assert data['success'] is True
    ...
    response = client.put(f'/api/evaluations/{evaluation_id}/status', headers=user_headers, json=status_data)
    assert response.status_code == 200
    data = json_response(response)
    assert data['success'] is True
    ...
    response = client.get(f'/api/evaluations/{evaluation_id}/logs', headers=admin_headers)
    assert response.status_code == 200
    data = json_response(response)
    assert data['success'] is True
...
def test_admin_user_management(client, session, admin_user, admin_headers):
    ...
    response = client.put(f'/api/users/{user_id}', headers=admin_headers, json=update_data)
    assert response.status_code == 200
    data = json_response(response)
    assert data['success'] is True
    assert data['data']['user']['fullName'] == update_data['fullName']
    ...
    response = client.put(f'/api/users/{user_id}/status', headers=admin_headers, json=status_data)
    assert response.status_code == 200
    data = json_response(response)
    assert data['success'] is True
