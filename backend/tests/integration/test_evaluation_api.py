...
def test_get_evaluation_detail(client, session, regular_user, user_headers):
    ...
    assert data['data']['evaluation']['evaluation_number'] == evaluation.evaluation_number
...
def test_create_evaluation(client, session, regular_user, user_headers):
    ...
    data = json_response(response)
    assert 'evaluation' in data['data']
    assert data['data']['evaluation']['evaluation_number'] == evaluation_data['evaluation_number']
    ...
    evaluation = session.query(Evaluation).filter_by(evaluation_number=evaluation_data['evaluation_number']).first()
...
def test_create_evaluation_auto_generated_number(client, session, regular_user, user_headers):
    ...
    # Verify that the evaluation was created in the database
    evaluation = session.query(Evaluation).filter_by(evaluation_number=evaluation_number).first()
    assert evaluation is not None
    assert evaluation.product_name == evaluation_data['product_name']
...
def test_create_evaluation_with_status(client, session, regular_user, user_headers):
    ...
    response = client.post('/api/evaluations', headers=user_headers, json=in_progress_data)
    assert response.status_code == 201
    data = json_response(response)
    assert data['data']['evaluation']['status'] == 'in_progress'


def test_create_multiple_evaluations_same_day(client, session, regular_user, user_headers):
    ...
    for i in range(3):
        ...
        response = client.post('/api/evaluations', headers=user_headers, json=evaluation_data)
        assert response.status_code == 201
        data = json_response(response)
...
def test_update_evaluation(client, session, regular_user, user_headers):
    ...
    # Create a test evaluation
    evaluation = create_test_evaluation(session, regular_user.id, status=EvaluationStatus.IN_PROGRESS.value)
...
def test_get_evaluation_logs(client, session, regular_user, admin_user, admin_headers):
    ...
    response = client.get(f'/api/evaluations/{evaluation.id}/logs', headers=admin_headers)
