...
class SystemConfig(db.Model):
    ...
    config_type = db.Column(
        db.Enum("string", "integer", "float", "boolean", "json", name="config_types"),
        nullable=False,
        default="string",
    )
    ...
    )  # 'general', 'backup', 'notification', etc.
    is_public = db.Column(
    ...
    def initialize_default_configs():
        ...
        default_configs = [
            ...
            # Evaluation settings
            (
                "evaluation_number_prefix",
                "EVAL",
                "string",
                "Evaluation number prefix",
                "evaluation",
                False,
            ),
            (
            ...
            (
                ...
                50,
                "integer",
                "Maximum active evaluations per user",
                "evaluation",
                False,
            ),
            # Security settings
        ...
        for (
            ...
            if not SystemConfig.query.filter_by(config_key=key).first():
                SystemConfig.set_config(
                    key, value, config_type, description, category, is_public
                )
