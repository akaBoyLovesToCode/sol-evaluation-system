...
def get_evaluation(evaluation_id):
    ...
    try:
        ...
        # Check permissions
        if not current_user.has_permission('part_leader') and evaluation.evaluator_id != current_user_id:
            return create_response(
        ...
        if evaluation.part_approver_id:
            ...
            eval_dict['part_approver_name'] = part_approver.full_name if part_approver else None
        
        if evaluation.group_approver_id:
            group_approver = User.query.get(evaluation.group_approver_id)
            eval_dict['group_approver_name'] = group_approver.full_name if group_approver else None
...
def update_evaluation(evaluation_id):
    ...
    try:
        ...
        # Check permissions
        can_edit = (
            evaluation.evaluator_id == current_user_id or  # Owner
            current_user.has_permission('admin') or  # Admin
            (current_user.has_permission('part_leader') and evaluation.status == 'in_progress')  # Leader for in-progress
        )
...
def approve_evaluation(evaluation_id):
    ...
    try:
        ...
        if evaluation.status == 'pending_part_approval' and current_user.has_permission('part_leader'):
            approval_level = 'part'
        elif evaluation.status == 'pending_group_approval' and current_user.has_permission('group_leader'):
        ...
        if evaluation.status == 'completed':
            ...
            group_leaders = User.query.filter_by(role='group_leader', is_active=True).all()
...
def reject_evaluation(evaluation_id):
    ...
    try:
        ...
        if not evaluation.can_be_approved_by(current_user):
            return create_response(
                message='Evaluation cannot be rejected by this user',
                status_code=400
            )
        ...
        if evaluation.remarks:
            evaluation.remarks += f'\n\nRejected by {current_user.full_name}: {rejection_reason}'
        else:
            evaluation.remarks = f'Rejected by {current_user.full_name}: {rejection_reason}'
        ...
        db.session.rollback()
        return create_response(
            message='Failed to reject evaluation',
            status_code=500
        )
...
def update_evaluation_status(evaluation_id):
    ...
    try:
        ...
        # Check permissions
        can_update = (
            evaluation.evaluator_id == current_user_id or
        ...
        if new_status == 'paused':
            ...
            evaluation.cancel()
        elif new_status == 'completed' and evaluation.evaluation_type == 'mass_production':
            evaluation.complete()
        else:
            return create_response(
