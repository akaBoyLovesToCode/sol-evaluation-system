...
def get_dashboard_overview():
    ...
    try:
        ...
        if not current_user.has_permission('part_leader'):
            status_counts = status_counts.filter(Evaluation.evaluator_id == current_user_id)
        ...
        if current_user.has_permission('part_leader'):
            ...
            for evaluation in pending_query.order_by(Evaluation.created_at.desc()).limit(5):
                eval_dict = evaluation.to_dict()
                eval_dict['evaluator_name'] = evaluation.evaluator.full_name
...
def get_detailed_statistics():
    ...
    try:
        ...
        evaluations_over_time = evaluations_over_time.group_by(
            date_trunc
        ...
        completion_rate_data = completion_rate_data.group_by(
            date_trunc
        ).order_by(date_trunc).all()
        ...
        if current_user.has_permission('part_leader'):
            evaluator_stats = db.session.query(
                User.full_name,
                ...
                func.avg(
                    func.datediff(
                        func.coalesce(Evaluation.completion_date, func.current_date()),
...
def get_evaluation_status_report():
    ...
    try:
        ...
        current_status = db.session.query(
            Evaluation.status,
            func.count(Evaluation.id).label('count'),
            func.round(
                func.count(Evaluation.id) * 100.0 / 
                func.sum(func.count(Evaluation.id)).over(), 2
            ).label('percentage')
        ).group_by(Evaluation.status).all()
        
        # Status distribution for last 30 days (for comparison)
        thirty_days_ago = datetime.now() - timedelta(days=30)
        previous_status = db.session.query(
            Evaluation.status,
            func.count(Evaluation.id).label('count')
        ).filter(
        ...
        pending_evaluations = db.session.query(
            ...
            func.datediff(func.current_date(), Evaluation.start_date).desc()
        ).limit(20).all()
        ...
        return create_response(
            data={
                ...
                'pending_evaluations': [
                    ...
                    for eval_id, eval_number, product, status, evaluator, days_pending 
                    in pending_evaluations
                ],
                'overdue_count': overdue_count,
                'summary': {
                    'total_active': sum(count for _, count, _ in current_status 
                                      if _ in ['in_progress', 'pending_part_approval', 'pending_group_approval']),
                    'total_completed': sum(count for _, count, _ in current_status 
                                         if _ == 'completed'),
        ...
        return create_response(
            message='Failed to retrieve evaluation status report',
            status_code=500
        )
...
def get_productivity_report():
    ...
    try:
        ...
        monthly_productivity = db.session.query(
            ...
            func.avg(
                func.case(
                    [(and_(Evaluation.status == 'completed', 
                           Evaluation.completion_date.isnot(None)),
                      func.datediff(Evaluation.completion_date, Evaluation.start_date))],
                    else_=None
                )
            ...
            func.date_format(Evaluation.start_date, '%Y-%m')
        ).order_by(
            func.date_format(Evaluation.start_date, '%Y-%m')
        ).all()
        ...
        department_performance = db.session.query(
            User.department,
            func.count(Evaluation.id).label('total_evaluations'),
            func.sum(
                func.case(
        ...
        return create_response(
            message='Failed to retrieve productivity report',
            status_code=500
        ) 