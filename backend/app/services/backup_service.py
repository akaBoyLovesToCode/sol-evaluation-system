...
class BackupService:
    ...
    def create_database_backup(
        ...
        try:
            ...
            if "mysql" in db_uri:
                ...
                return (
                    True,
                    f"Database backup created successfully: {backup_name}",
                    backup_path,
                )
    ...
    def create_files_backup(
        ...
        try:
            ...
            return (
                True,
                f"Files backup created successfully: {backup_name}",
                backup_path,
            )
    ...
    def create_full_backup(
        ...
        try:
            ...
            db_success, db_message, db_path = BackupService.create_database_backup(
                f"{backup_name}_db"
            )
            ...
            # Create files backup
            files_success, files_message, files_path = (
                BackupService.create_files_backup(f"{backup_name}_files")
            )
            ...
            if db_success and files_success:
                return (
                    True,
                    f"Full backup created successfully: {backup_name}",
                    backup_paths,
                )
            elif db_success:
                return (
                    True,
                    f"Partial backup created (database only): {db_message}",
                    backup_paths,
                )
            elif files_success:
                return (
                    True,
                    f"Partial backup created (files only): {files_message}",
                    backup_paths,
                )
            else:
                return (
                    False,
                    f"Backup failed: DB - {db_message}, Files - {files_message}",
                    backup_paths,
                )
                ...
                if filename.endswith(".json"):
                    ...
                    try:
                        ...
                        current_app.logger.warning(
                            f"Error reading backup metadata {filename}: {str(e)}"
                        )
                        continue
        ...
        try:
            ...
            if deleted_files:
                current_app.logger.info(
                    f"Deleted backup files: {', '.join(deleted_files)}"
                )
            ...
            for backup in backups:
                try:
                    ...
                    if created_at < cutoff_date:
                        success, message = BackupService.delete_backup(
                            backup["backup_name"]
                        )
                        if success:
                            deleted_count += 1
                    ...
                    current_app.logger.warning(
                        f"Error processing backup {backup.get('backup_name', 'unknown')}: {str(e)}"
                    )
                    continue
    ...
    def get_backup_statistics() -> dict:
        ...
        try:
            ...
            stats = {
                ...
                "database_backups": len(
                    [b for b in backups if b.get("backup_type") == "database"]
                ),
                "files_backups": len(
                    [b for b in backups if b.get("backup_type") == "files"]
                ),
                "total_size_bytes": sum(
                    [
                        b.get("file_size", 0)
                        for b in backups
                        if b.get("file_exists", False)
                    ]
                ),
                "oldest_backup": None,
                "newest_backup": None,
                "missing_files": len(
                    [b for b in backups if not b.get("file_exists", False)]
                ),
            }
        ...
        try:
            ...
            if backups:
                ...
                if (datetime.utcnow() - latest_date).days < backup_frequency:
                    current_app.logger.info(
                        f"Automatic backup not needed yet (last backup: {latest_date})"
                    )
                    return True
