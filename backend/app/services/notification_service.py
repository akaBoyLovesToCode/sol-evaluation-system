...
class NotificationService:
    ...
    @staticmethod
    def send_notification(sender_id: int, recipient_id: int, title: str, 
                         content: str, message_type: MessageType = MessageType.SYSTEM,
                         related_evaluation_id: Optional[int] = None) -> bool:
        """
        ...
        try:
            ...
            if not sender or not recipient:
                current_app.logger.error(f"Invalid sender ({sender_id}) or recipient ({recipient_id})")
                return False
            ...
            current_app.logger.info(f"Notification sent from {sender.username} to {recipient.username}")
            return True
    ...
    @staticmethod
    def send_bulk_notification(sender_id: int, recipient_ids: List[int], 
                              title: str, content: str, 
                              message_type: MessageType = MessageType.SYSTEM,
                              related_evaluation_id: Optional[int] = None) -> int:
        """
    ...
    def get_user_notifications(user_id: int, limit: int = 50, 
                              status_filter: Optional[MessageStatus] = None) -> List[Dict]:
        """
        ...
        try:
            ...
            for message in messages:
                result.append({
                    ...
                    'related_evaluation_title': message.related_evaluation.title if message.related_evaluation else None
                })
    ...
    def send_daily_digest(user_id: int) -> bool:
        ...
        try:
            ...
            if new_assignments > 0:
                digest_items.append(f"• {new_assignments} new evaluation(s) assigned to you")
            ...
            if user.role in [UserRole.PART_LEADER, UserRole.GROUP_LEADER]:
                ...
                if pending_approvals > 0:
                    digest_items.append(f"• {pending_approvals} evaluation(s) pending your approval")
        ...
        try:
            ...
            if read_messages:
                total_response_time = sum([
                    (msg.read_at - msg.created_at).total_seconds() / 3600  # Convert to hours
                    for msg in read_messages
                ])
                stats['average_response_hours'] = round(total_response_time / len(read_messages), 1)
            else:
