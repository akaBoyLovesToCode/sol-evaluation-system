...
class AnalyticsService:
    ...
    @staticmethod
    def get_evaluation_statistics(start_date: Optional[datetime] = None, 
        ...
        try:
            ...
            # Completion rate
            completed_count = base_query.filter_by(status=EvaluationStatus.COMPLETED).count()
            stats['completion_rate'] = round(
                (completed_count / stats['total_evaluations'] * 100) if stats['total_evaluations'] > 0 else 0, 
                2
            )
            ...
            if completed_evals:
                ...
                stats['average_completion_days'] = round(total_days / len(completed_evals), 1)
            else:
            ...
            stats['overdue_rate'] = round(
                (overdue_count / stats['total_evaluations'] * 100) if stats['total_evaluations'] > 0 else 0,
                2
            )
    ...
    def get_monthly_trends(months: int = 12) -> Dict:
        ...
        try:
            ...
            monthly_data = db.session.query(
                ...
                func.sum(func.case([(Evaluation.status == EvaluationStatus.COMPLETED, 1)], else_=0)).label('completed'),
                func.sum(func.case([(Evaluation.status == EvaluationStatus.REJECTED, 1)], else_=0)).label('rejected')
            ).filter(
                Evaluation.created_at >= start_date
            ).group_by(
            ...
            for row in monthly_data:
                ...
                completion_rate = round(
                    (row.completed / row.total * 100) if row.total > 0 else 0,
                    1
                )
    ...
    def get_user_performance_metrics() -> List[Dict]:
        ...
        try:
            ...
            for user in users:
                ...
                completed_assigned = len([e for e in assigned_evals if e.status == EvaluationStatus.COMPLETED])
                overdue_assigned = len([
                    e for e in assigned_evals 
                    if e.due_date and e.due_date < datetime.utcnow().date() and e.status != EvaluationStatus.COMPLETED
                ])
            ...
            current_app.logger.error(f"Error getting user performance metrics: {str(e)}")
            return []
    ...
    def get_evaluation_type_analysis() -> Dict:
        ...
        try:
            ...
            for eval_type in EvaluationType:
                ...
                completed_count = len([e for e in type_evals if e.status == EvaluationStatus.COMPLETED])
                rejected_count = len([e for e in type_evals if e.status == EvaluationStatus.REJECTED])
                
                # Average completion time
                completed_with_time = [
                    e for e in type_evals 
                    if e.status == EvaluationStatus.COMPLETED and e.completed_at
                ]
                ...
                success_rate = round(
                    (completed_count / total_finished * 100) if total_finished > 0 else 0,
                    1
                )
            ...
            current_app.logger.error(f"Error getting evaluation type analysis: {str(e)}")
            return {}
    ...
    def get_workflow_bottlenecks() -> Dict:
        ...
        try:
            ...
            user_workloads = db.session.query(
                User.id,
                User.username,
                func.count(Evaluation.id).label('active_count')
            ).join(
                Evaluation, Evaluation.assigned_to == User.id
            ).filter(
                Evaluation.status.in_([EvaluationStatus.PENDING, EvaluationStatus.IN_PROGRESS])
            ).group_by(
    ...
    def generate_dashboard_data() -> Dict:
        ...
        try:
            ...
            dashboard_data['type_analysis'] = AnalyticsService.get_evaluation_type_analysis()
    ...
    @staticmethod
    def export_analytics_data(start_date: datetime, end_date: datetime, 
                             format_type: str = 'json') -> Tuple[bool, str, Optional[str]]:
        """
        ...
        try:
            ...
            export_data = {
                ...
                'statistics': AnalyticsService.get_evaluation_statistics(start_date, end_date),
            ...
            if format_type.lower() == 'json':
                return True, "Data exported successfully", json.dumps(export_data, indent=2)
    ...
    def _convert_to_csv(data: Dict) -> str:
        ...
        try:
            ...
            csv_lines.append("Username,Role,Total Assigned,Completed,Completion Rate,Avg Days")
        ...
        try:
            ...
            for eval_type in EvaluationType:
                ...
                if len(completed_evals) >= 3:  # Need minimum data for prediction
                    ...
                    workload_factor = 1 + (current_workload * 0.1)  # 10% increase per pending evaluation
