...
class WorkflowService:
    ...
    VALID_TRANSITIONS = {
        EvaluationStatus.IN_PROGRESS: [
            EvaluationStatus.PENDING_PART_APPROVAL,
            EvaluationStatus.COMPLETED,
            EvaluationStatus.PAUSED,
            EvaluationStatus.CANCELLED,
        ],
        EvaluationStatus.PENDING_PART_APPROVAL: [
            EvaluationStatus.PENDING_GROUP_APPROVAL,
    ...
    APPROVAL_REQUIREMENTS = {
        (EvaluationStatus.IN_PROGRESS, EvaluationStatus.PENDING_PART_APPROVAL): [
            UserRole.USER,
        ...
        (
            EvaluationStatus.PENDING_PART_APPROVAL,
            EvaluationStatus.PENDING_GROUP_APPROVAL,
        ): [UserRole.PART_LEADER, UserRole.GROUP_LEADER, UserRole.ADMIN],
        (EvaluationStatus.PENDING_GROUP_APPROVAL, EvaluationStatus.COMPLETED): [
            UserRole.GROUP_LEADER,
            UserRole.ADMIN,
        ],
        (EvaluationStatus.IN_PROGRESS, EvaluationStatus.COMPLETED): [
            UserRole.GROUP_LEADER,
            UserRole.ADMIN,
        ],  # For mass production
        (EvaluationStatus.PENDING_PART_APPROVAL, EvaluationStatus.REJECTED): [
            UserRole.PART_LEADER,
            UserRole.GROUP_LEADER,
            UserRole.ADMIN,
        ],
        (EvaluationStatus.PENDING_GROUP_APPROVAL, EvaluationStatus.REJECTED): [
            UserRole.GROUP_LEADER,
            UserRole.ADMIN,
        ],
        (EvaluationStatus.IN_PROGRESS, EvaluationStatus.PAUSED): [
            UserRole.USER,
            UserRole.PART_LEADER,
            UserRole.GROUP_LEADER,
            UserRole.ADMIN,
        ],
        (EvaluationStatus.IN_PROGRESS, EvaluationStatus.CANCELLED): [
            UserRole.PART_LEADER,
            UserRole.GROUP_LEADER,
            UserRole.ADMIN,
        ],
        (EvaluationStatus.PAUSED, EvaluationStatus.IN_PROGRESS): [
            UserRole.USER,
            UserRole.PART_LEADER,
            UserRole.GROUP_LEADER,
            UserRole.ADMIN,
    ...
    def can_transition(
        ...
        if required_roles and user_role not in required_roles:
            return (
                False,
                f"User role {user_role.value} not authorized for this transition",
            )
    ...
    def transition_status(
        ...
        try:
            ...
            WorkflowService._send_status_change_notifications(
                evaluation, old_status, new_status.value, user
            )
    ...
    def _send_status_change_notifications(
        ...
        try:
            ...
            if new_status == "pending_part_approval":
                ...
                if (
                    evaluation.part_approver_id
                    and evaluation.part_approver_id != actor.id
                ):
                ...
                if (
                    evaluation.group_approver_id
                    and evaluation.group_approver_id != actor.id
                ):
            ...
            current_app.logger.error(
                f"Error sending status change notifications: {str(e)}"
            )
    ...
    def get_pending_approvals(user_id: int) -> List[Dict]:
        ...
        try:
            ...
            if user.role == "part_leader":
                ...
                query = query.filter(Evaluation.status == "pending_part_approval")
            elif user.role == "group_leader":
                # Group leaders see evaluations pending group approval
                query = query.filter(Evaluation.status == "pending_group_approval")
            else:
                ...
                result.append(
                    {
                        ...
                        "start_date": eval.start_date.isoformat()
                        if eval.start_date
                        else None,
        ...
        try:
            ...
            if completed_evals:
                ...
                stats["average_completion_days"] = (
                    round(total_days / len(completed_evals), 1) if total_days > 0 else 0
                )
            else:
